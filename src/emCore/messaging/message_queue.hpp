...
private:
    ...
    os::task_handle_t owner_handle_{nullptr};
    u16 owner_id_{0};
...
public:
    ...
    result<void, error_code> send(const MessageType& msg) noexcept {
        ...
        if (owner_handle_ != nullptr) {
            os::notify_task(owner_handle_, 0x01);
        }
        return ok();
    }
    ...
    result<MessageType, error_code> receive_wait(u32 timeout_ms) noexcept {
        ...
        bool notified = os::wait_notification(timeout_ms, &notification_value);
